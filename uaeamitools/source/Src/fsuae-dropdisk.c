/* 
    fsuae-dropdisk.c
    by T.J.Roughton
    (for exclusive use with my alterations to fs-uae)
*/
...
** little or no work.
*/
VOID handleGadgetEvent(struct Window *win, struct Gadget *gad, UWORD code, WORD *slider_level, struct Gadget *my_gads[]) {
...
*/
struct Gadget *createAllGadgets(struct Gadget **glistptr, void *vi, UWORD topborder, WORD slider_level, struct Gadget *my_gads[]) {
...
*/
VOID process_window_events(struct Window *mywin, WORD *slider_level, struct Gadget *my_gads[]) {
  ...
  if (NULL == (font = OpenFont(&Topaz80))) {
    ...
    if (NULL == (mysc = LockPubScreen(NULL))) {
        ...
        if (NULL == (vi = GetVisualInfo(mysc, TAG_END))) {
           ...
           if (NULL == createAllGadgets(&glist, vi, topborder, slider_level, my_gads)) {
